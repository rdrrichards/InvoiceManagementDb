...
CREATE PROCEDURE [dbo].[usp_get_next_invoice_num]
	@entity_key int
AS
...
BEGIN
	...
	AND B.attribute_type_code = 'INVNUM'
...
END
	...
	BEGIN
		ROLLBACK
		EXEC dbo.usp_LogError 'attribute', 'usp_get_next_invoice_num', 'U'
		--RETURN -1
	END
	ELSE
		COMMIT
...
WHERE A.entity_key = @entity_key
AND A.entity_type_key = 3 -- account
AND B.attribute_type_code = 'INVNUMPAT'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_get_next_invoice_num] TO [businessuser]
    AS [dbo];
